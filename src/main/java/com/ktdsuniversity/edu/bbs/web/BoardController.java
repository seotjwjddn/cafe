...
public class BoardController {
	...
	// @Valid 바로 다음에 있어야 한다. (순서가 매우 중요하다.)
		...
		// 게시글을 등록한다.
		boolean isSucces = boardService.createNewBoard(boardVO, file);
		if (isSucces) {
			// 게시글의 등록 결과가 성공이라면
			// /board/list URL로 이동한다.
			// redirect:는 화면을 이동시키는 View 다.
			// redirect 로 화면을 전환시킬 때에는
			// addObject를 사용할 수 없다.
			...
			modelAndView.setViewName("board/boardwrite");
			modelAndView.addObject("boardwrite", boardVO);
			return modelAndView;
		}

	}

	// http://localhost:8080/board/view?id=1 (?id=1 -> Query String)
	...
	public ModelAndView viewOneBoard(@RequestParam int id) {
		...
		BoardVO boardVO = boardService.getOneBoardVO(id, true);
	...
	public ResponseEntity<Resource> downloadFile(@PathVariable int id) {
		...
		BoardVO boardVO = boardService.getOneBoardVO(id, false);
		if (boardVO == null) {
	...
	public String doBoardUpdate(@Valid @ModelAttribute BoardVO boardVO
